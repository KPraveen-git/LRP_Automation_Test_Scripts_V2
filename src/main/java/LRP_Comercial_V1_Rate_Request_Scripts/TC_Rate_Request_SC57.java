...
public class TC_Rate_Request_SC57  extends Keywords {
	public static String Pre_Carriage_Radiobtn;
	public static String On_Carriage_Radiobtn;
	public static String tService;
	public static String TOS;

	public void Rate_Request_SC57(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset)
			throws Exception {

		String testCase_Name="TC_Rate_Request_SC57";
		...
		if(isdisplayed(driver,pop_up_exp )) {
			...
			if(isdisplayed(driver,pop_up_exp )) {
				...
				waitForElement(driver, SRR_Port_Pair_Table_Headers);
				List<Map<String, String>> SRR_Table_Value = getTableData(driver, SRR_Port_Pair_Table_Headers, SRR_Port_Pair_Table_Rows);
				String actual_From_Date = getFirstValueByHeader(SRR_Table_Value, "Valid From");
				...
				if(isdisplayed(driver,pop_up_exp )) {
					...
					waitForElement(driver, SRR_Port_Pair_Table_Headers);
					SRR_Table_Value = getTableData(driver, SRR_Port_Pair_Table_Headers, SRR_Port_Pair_Table_Rows);
					String actual_To_Date1 = getFirstValueByHeader(SRR_Table_Value, "Valid To");
					if(isSecondDateBeforeFirst(actual_From_Date, actual_To_Date1)) {
						System.out.println("Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date1+" changed to one day before the valid from2 "+actual_From_Date);
						Extent_pass(driver, "Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date1+" changed to one day before the valid from2 "+actual_From_Date, test, test1);
					}else {
						System.out.println("Not Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date1+" Not changed to one day before the valid from2 "+actual_From_Date);
						Extent_fail(driver, "Not Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date1+" Not changed to one day before the valid from2 "+actual_From_Date, test, test1);
