...
public class TC_Rate_Request_SC49 extends Keywords{

	public void Rate_Request_SC49(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset){


		String testCase_Name="TC_Rate_Request_SC49";
		...
		String Exp_Det_Days_Input_1 = Data.get("Exp_Det_Days_Input_1");
		String Imp_Det_Days_Input_1 = Data.get("Imp_Det_Days_Input_1");
		String Exp_Demmurage_Input_1 = Data.get("Exp_Demmurage_Input_1");
		String Imp_Demmurage_Input_1 = Data.get("Imp_Demmurage_Input_1");
...
ResetProfile(driver);
				...
				Step_Start(19, "Change the detention and demurrage days.", test, test1);

				mouseOverToElement(driver, export_detention_txtfld_Srr);
				
				waitForElement(driver, export_detention_txtfld_Srr);
				clear(driver, export_detention_txtfld_Srr);
				sendKeys(driver, export_detention_txtfld_Srr, Exp_Det_Days_Input_1);
		
				waitForElement(driver, import_detention_txtfld_Srr);
				clear(driver, import_detention_txtfld_Srr);
				sendKeys(driver, import_detention_txtfld_Srr, Imp_Det_Days_Input_1);
		
				waitForElement(driver, export_demmurage_txtfld_Srr);
				clear(driver, export_demmurage_txtfld_Srr);
				sendKeys(driver, export_demmurage_txtfld_Srr, Exp_Demmurage_Input_1);
		
				waitForElement(driver, import_demmurage_txtfld_Srr);
				clear(driver, import_demmurage_txtfld_Srr);
				sendKeys(driver, import_demmurage_txtfld_Srr, Imp_Demmurage_Input_1);
				
				Step_End(19, "Change the detention and demurrage days.", test, test1);


				Step_Start(20, "Enter the OFR amount.", test, test1);
				...
				Step_End(20, "Enter the OFR amount.", test, test1);

				Step_Start(21, "Enter the Remarks.", test, test1);

		scrollBottom(driver);
		waitForElement(driver, remarks_txtfld_Srr);
		sendKeys(driver, remarks_txtfld_Srr, remarks_txtfld_data);
		
		Step_End(21, "Enter the Remarks.", test, test1);

		Step_Start(22, ".Click Apply rate button.", test, test1);
		scrollTop(driver);
		waitForElement(driver, apply_rate_Btn);
		click(driver, apply_rate_Btn);
		
		Step_End(22, ".Click Apply rate button.", test, test1);
		...
		if(port_pair_Status_actual.equals(port_pair_Status_exp)) {
			...
			Extent_call(test, test1, "After saving the SRR the free days updated in the system to be checked with the excel sheet");
		scrollBottom(driver);
		mouseOverToElement(driver, export_detention_txtfld_Srr);
		String exp_det_actual = getAttribute(driver, export_detention_txtfld_Srr, "value");
		String imp_det_actual = getAttribute(driver, import_detention_txtfld_Srr, "value");
		String exp_demm_actual = getAttribute(driver, export_demmurage_txtfld_Srr, "value");
		String imp_demm_actual = getAttribute(driver, import_demmurage_txtfld_Srr, "value");

		
		if(exp_det_actual.equals(Exp_Det_Days_Input_1)) {
			
			System.out.println("Matched || Expected export detention days -> "+Exp_Det_Days_Input_1+"|| actual export detention days -> "+exp_det_actual);
			Extent_pass(driver, "Matched || Expected export detention days -> "+Exp_Det_Days_Input_1+"|| actual export detention days -> "+exp_det_actual, test, test1);
		}else {
			
			System.out.println("Matched || Expected export detention days -> "+Exp_Det_Days_Input_1+"|| actual export detention days -> "+exp_det_actual);
			Extent_fail(driver, "Matched || Expected export detention days -> "+Exp_Det_Days_Input_1+"|| actual export detention days -> "+exp_det_actual, test, test1);
			
		}
		
		
	if(imp_det_actual.equals(Imp_Det_Days_Input_1)) {
			
			System.out.println("Matched || Expected import detention days -> "+Imp_Det_Days_Input_1+"|| actual import detention days -> "+imp_det_actual);
			Extent_pass(driver, "Matched || Expected import detention days -> "+Imp_Det_Days_Input_1+"|| actual import detention days -> "+imp_det_actual, test, test1);
		}else {
			
			System.out.println("Matched || Expected import detention days -> "+Imp_Det_Days_Input_1+"|| actual import detention days -> "+imp_det_actual);
			Extent_fail(driver, "Matched || Expected import detention days -> "+Imp_Det_Days_Input_1+"|| actual import detention days -> "+imp_det_actual, test, test1);
			
		}
	
	
	if(exp_demm_actual.equals(Exp_Demmurage_Input_1)) {
			
			System.out.println("Matched || Expected export demmurage days -> "+Exp_Demmurage_Input_1+"|| actual export demmurage days -> "+exp_demm_actual);
			Extent_pass(driver, "Matched || Expected export demmurage days -> "+Exp_Demmurage_Input_1+"|| actual export demmurage days -> "+exp_demm_actual, test, test1);
		}else {
			
			System.out.println("Matched || Expected export demmurage days -> "+Exp_Demmurage_Input_1+"|| actual export demmurage days -> "+exp_demm_actual);
			Extent_fail(driver, "Matched || Expected export demmurage days -> "+Exp_Demmurage_Input_1+"|| actual export demmurage days -> "+exp_demm_actual, test, test1);
			
		}
	
	
	if(imp_demm_actual.equals(Imp_Demmurage_Input_1)) {
		
		System.out.println("Matched || Expected import demmurage days -> "+Imp_Demmurage_Input_1+"|| actual import demmurage days -> "+imp_demm_actual);
		Extent_pass(driver, "Matched || Expected import demmurage days -> "+Imp_Demmurage_Input_1+"|| actual import demmurage days -> "+imp_demm_actual, test, test1);
	}else {
		
		System.out.println("Matched || Expected import demmurage days -> "+Imp_Demmurage_Input_1+"|| actual import demmurage days -> "+imp_demm_actual);
		Extent_fail(driver, "Matched || Expected import demmurage days -> "+Imp_Demmurage_Input_1+"|| actual import demmurage days -> "+imp_demm_actual, test, test1);
