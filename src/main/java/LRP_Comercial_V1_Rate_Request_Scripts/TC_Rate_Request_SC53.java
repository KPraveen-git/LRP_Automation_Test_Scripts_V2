...
public class TC_Rate_Request_SC53  extends Keywords{
	...
	public void Rate_Request_SC53(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset)
			throws Exception {
		
		String tc_Name="TC_Rate_Request_SC53";
		...
		String To_Date_SRR=Data.get("To_Date_SRR");
...
moduleNavigate(driver, rateRequestModule);
		...
		if(isDisplayed(driver,pop_up_exp )) {
			...
			if (actRequestStatus.equals(requestStatus)) {
				...
				waitForElement(driver, Request_Input_select);
				click(driver, Request_Input_select);
				...
				waitForElement(driver, SRR_edit_button);
				click(driver, SRR_edit_button);
				Step_End(17, "Click edit", test, test1);
				waitForElement(driver, valid_to);
				String Valid_to_date=getText(driver, valid_to);
				System.out.println(Valid_to_date);
				...
				waitForElement(driver, first_grid_click);
				doubleClick(driver, first_grid_click);
				...
				Step_Start(21, "click To date button and enter the valid to date.", test, test1);
				waitForElement(driver, To_Date_click);
			click(driver, To_Date_click);

			waitForElement(driver, To_Date_input);
	clearAndType(driver, To_Date_input, To_Date_SRR);
	waitForElement(driver, To_Date_input_ok);
	click(driver, To_Date_input_ok);
	Step_End(21, "click To date button and enter the valid to date.", test, test1);
	Step_Start(22, "click save..", test, test1);
	waitForElement(driver, toolBar_Save_Button);
	click(driver, toolBar_Save_Button);	
	Step_End(22, "click save..", test, test1);
	
	String updated_pop_act1 = getText(driver, popup_5_Sec);
	System.out.println(updated_pop_act1);
	
	if(Updated_pop_Exp.equals(updated_pop_act1)) {
		Extent_pass(driver,"Matched || Expected : "+Updated_pop_Exp+" Actual : "+ updated_pop_act1, test,test1);
		System.out.println("Matched || Expected : "+Updated_pop_Exp+" Actual : "+ updated_pop_act1);
		Extent_pass(driver, Updated_pop_Exp+" is displayed", test,test1);
		System.out.println(Updated_pop_Exp+" is displayed");

	}else {
		System.out.println("Not Matched || Expected : "+wantToSavePopup+" Actual : "+ updated_pop_act1);

		Extent_fail(driver,"Not Matched || Expected : "+wantToSavePopup+" Actual : "+ updated_pop_act1, test,test1);
	}
	
	String Valid_to_date1=getText(driver, valid_to);
	System.out.println(Valid_to_date1);
	
	
	if(!Valid_to_date.equals(Valid_to_date1)) {
		
		Extent_pass(driver, "Matched || Expected Before change : "+Valid_to_date+" || Actual After change : "+Valid_to_date1, test, test1);
		System.out.println("Matched || Expected  Before change : "+Valid_to_date+" || Actual After change : "+Valid_to_date1);
		
	}else {
		System.out.println("Not Matched || Expected  Before change : "+Valid_to_date+" || Actual After change : "+Valid_to_date1);
		Extent_fail(driver, "Not Matched || Expected  Before change : "+Valid_to_date+" || Actual After change : "+Valid_to_date1, test, test1);
	}
