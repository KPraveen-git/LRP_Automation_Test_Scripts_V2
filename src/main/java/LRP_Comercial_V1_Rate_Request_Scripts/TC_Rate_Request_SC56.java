...
public class TC_Rate_Request_SC56 extends Keywords {
	...
	public void Rate_Request_SC56(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset)
			throws Exception {
		
		String tc_Name="TC_Rate_Request_SC56";
...
moduleNavigate(driver, rateRequestModule);
		...
		waitForDisplay(driver, Exp_Days);
			...
			if (actRequestStatus.equals(requestStatus)) {
				...
				Step_Start(17, "click contribution button.", test, test1);
				waitForElement(driver, Contribute_button);
			click(driver, Contribute_button);
			Step_End(17, "click contribution button.", test, test1);
			
			
			waitForElement(driver, Ref_Number_CONTRIBUTE);
				String ref_number_field = getAttribute(driver, Ref_Number_CONTRIBUTE,"value");
				System.out.println(ref_number_field);
	
	Extent_pass(driver, "Ref number was shown as ****"+ref_number_field, test, test1);
