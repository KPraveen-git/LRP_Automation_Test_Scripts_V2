...
public class TC_Rate_Request_SC46 extends Keywords {

	public void Rate_Request_SC46(WebDriver driver, ExtentTest test,ExtentTest test1, String selected_dataset) {
		String testCaseName="TC_Rate_Request_SC46";
		...
		// Rate Request
				...
				waitForElement(driver, Special_RR_Header);
				List<Map<String, String>> srr_Amount_Table_Data = getTableData(driver, Special_RR_Header, Special_RR_Row);
                String Expected_Amount = getLastRowValueByHeader(srr_Amount_Table_Data, "Usd Amt.");
				System.out.println("Expected_Amount :"+Expected_Amount);
				
				if(OFR_Input_SRR.equals(Expected_Amount)) {
					System.out.println("Matched || Expected status : "+OFR_Input_SRR+" || Actual status : "+Expected_Amount);
					Extent_pass(driver, "Matched || Expected status : "+OFR_Input_SRR+" || Actual status : "+Expected_Amount, test, test1);
				}else {
					System.out.println("Not Matched || " + " Expected status : " + OFR_Input_SRR+ " || Actual status : " + Expected_Amount);
					Extent_fail(driver, "Not Matched || " + " Expected status : " + OFR_Input_SRR+ " || Actual status : " + Expected_Amount, test,test1);
				}
