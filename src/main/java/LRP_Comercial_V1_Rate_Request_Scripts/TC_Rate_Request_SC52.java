...
public class TC_Rate_Request_SC52 extends Keywords{
	...
	public void Rate_Request_SC52(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset){
		...
		if(isDisplayed(driver,pop_up_exp )) {
			...
			ResetProfile(driver);
				...
				if(srr_status_actual_approval.equalsIgnoreCase(srr_Status_exp)) {
					...
					Extent_pass(driver, "Matched|| expected srr status is -> "+srr_Status_exp+"||actual srr status -> "+srr_status_actual_approval, test, test1);
				...
				if(srr_status_actual_recall.equalsIgnoreCase(requestStatus)) {
					...
					Extent_pass(driver, "Matched|| expected srr status is -> "+requestStatus+"||actual srr status -> "+srr_status_actual_recall, test, test1);
