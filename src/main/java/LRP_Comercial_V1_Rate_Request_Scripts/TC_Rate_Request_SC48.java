...
public class TC_Rate_Request_SC48 extends Keywords{
	...
	public void Rate_Request_SC48(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset) {
		...
		String surcharge_value=Data.get("surcharge_value");
		String Condition=Data.get("Condition");
		String amount_data=Data.get("amount_data");
		...
		if(isDisplayed(driver,pop_up_exp )) {
			...
			ResetProfile(driver);
				...
				Step_Start(19, ".By clicking + button system will display the surcharge and the select a surcharge.", test, test1);
				
				waitForElement(driver, surcharges_Add_button);
			click(driver, surcharges_Add_button);
			
			Step_End(19, ".By clicking + button system will display the surcharge and the select a surcharge.", test, test1);

			selectValue1(driver, Condition, surcharge_value);
			
			Step_Start(20, ".Enter the amount in the Agecncy Loc column.", test, test1);

			waitForDisplay(driver, surcharge_details_table_Srr);
			mouseOverToElement(driver, surcharge_details_table_Srr);
			
			String surcharge_to_edit = String.format(surcharge_amount_field, surcharge_value);
			waitForElement(driver, surcharge_to_edit);
			click(driver, surcharge_to_edit);
			
			
			String surcharge_Amnt_Input =  String.format(Rate_Request_Loactors.surcharge_Amount_Input,  surcharge_value); 
			...
			waitForDisplay(driver, ofr_txtfld_Srr);
			...
			Step_End(21, ".Enter the OFR amount.", test, test1);

			scrollBottom(driver);
			Step_Start(22, "Enter the Remarks.", test, test1);

			waitForElement(driver, remarks_txtfld_Srr);
			sendKeys(driver, remarks_txtfld_Srr, remarks_txtfld_data);
			
			Step_End(22, "Enter the Remarks.", test, test1);

			Step_Start(23, ".Click Apply rate button.", test, test1);

			scrollTop(driver);
			waitForElement(driver, apply_rate_Btn);
			click(driver, apply_rate_Btn);
			
			Step_End(23, ".Click Apply rate button.", test, test1);

			Step_Start(24, "Click save.", test, test1);
			waitForElement(driver, toolBar_save_Button);
			click(driver, toolBar_save_Button);
			
			Step_End(24, "Click save.", test, test1);
