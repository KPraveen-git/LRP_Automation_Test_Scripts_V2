...
public class TC_Cost_Activity_Report_TS052 extends Keywords {

	public void Cost_Activity_Report_TS052(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset)
			throws Exception {

		String tc_Name = "TC_Cost_Activity_Report_TS052";
		...
		Step_Start(11, "Click on the approve expense tab", test, test1);

		Step_Start(16, "Click on the open expense tab", test, test1);

		click(driver, ApprovedExpenses);

		Step_End(11, "Click on the approve expense tab", test, test1);
		Step_End(16, "Click on the open expense tab", test, test1);
		...
		for (WebElement approveddata : approved_rows) {
			...
			if (!Fieldcolor.equals(ColorCode) || !Fieldcolor.equals(disbursement_ColorCode)) {
				...
				Step_Start(13, "Click on the reject option", test, test1);

				Step_Start(17, "Select the required activities and click on the reject option", test, test1);

				waitForElement(driver, RejectExpense);
				click(driver, RejectExpense);

				Step_End(13, "Click on the reject option", test, test1);

				Step_End(17, "Select the required activities and click on the reject option", test, test1);
		...
		Step_Start(14,
				"If the activities are not invoiced and disbursement is not taken, then the system validates as Expenses are rejected",
				test, test1);

		Step_Start(18,
				"If the activities are in draft status, the system validates as Expenses are rejected Click on the ok option",
				test, test1);

		Step_Start(19,
				"If the activities are in submitted status, the system validates as Expenses are rejected Click on the ok option",
				test, test1);

		Step_Start(20,
				"If the activities are in rejected status, the system validates as Expenses are rejected, Click on the ok option",
				test, test1);
