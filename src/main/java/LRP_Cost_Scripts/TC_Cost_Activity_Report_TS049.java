...
public class TC_Cost_Activity_Report_TS049 extends Keywords {
	...
	public void Cost_Activity_Report_TS049(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset)
		...
		for (int i = 0; i < OE_Head_field.size(); i++) {
			...
			try {
				WebElement disbursementstatus = driver.findElement(By
						.xpath("(//div[@id='CRR_jTabbedPaneExpenses']//div[@col-id='sendToWDL' and @role='gridcell'])["
								+ (i + 1) + "]//input[@checked='checked']"));

				if (disbursementstatus.isDisplayed()) {
					OE_Checbox.add("Yes");
				} else {
					OE_Checbox.add("No");
				}
		...
		for (int i = 0; i < AE_Head_field.size(); i++) {
			...
			try {
				WebElement disbursementstatus = driver.findElement(
						By.xpath("(//div[@id='CRR_treeTableEXPDB']//div[@col-id='sendToWDL' and @role='gridcell'])["
								+ (i + 1) + "]//input[@checked='checked']"));
